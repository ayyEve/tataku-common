    ...
    fn on_click(&mut self, pos:Vector2<f64>, button:MouseButton, game:Arc<Mutex<&mut Game>>) {
        ...
        if let Some(clicked_tag) = self.beatmap_scroll.on_click(pos, button, game.clone()) {
            ...
            if *play {
                ...
                map.song.upgrade().map(|x| { x.pause(); x.set_position(0.0); } );
                map.reset();
            ...
            if let Some((b, _play)) = self.get_selected() {
                b.lock().unwrap().song.upgrade().map(|x| { x.pause(); x.set_position(0.0); });
            }
...
impl ScrollableItem for BeatmapsetItem {
    ...
    fn set_tag(&mut self, _tag:&str) {
        ...
        self.first.lock().unwrap().song.upgrade().map(|x| { x.pause(); x.set_position(0.0); });
    ...
    fn on_click(&mut self, pos:Vector2<f64>, _button:MouseButton) -> bool {
        ...
        if !self.selected && self.hover {
            // start song
            let song = self.first.lock().unwrap().song.upgrade().unwrap();
            song.play();
    ...
    fn on_volume_change(&mut self) {
        self.first.lock().unwrap().song.upgrade().unwrap().set_volume(Settings::get().get_music_vol());
    }

    fn dispose(&mut self) {
        self.first.lock().unwrap().song.upgrade().map(|x| { x.pause(); });
    }
}
