...
impl Serializable for SpectatorFrameData {
    fn read(sr:&mut crate::serialization::SerializationReader) -> Self {
        let r = match sr.read_u8() {
            ...
            // host changing map
            8 => SpectatorFrameData::ChangingMap,

            // responding with info
            9 => SpectatorFrameData::PlayingResponse {
                user_id:sr.read(), 
                beatmap_hash:sr.read(), 
                mode:sr.read(), 
                mods:sr.read(), 
                current_time:sr.read()
            },
    ...
    fn write(&self, sw:&mut crate::serialization::SerializationWriter) {
        match &self {
            ...
            SpectatorFrameData::ChangingMap => sw.write_u8(8),

            SpectatorFrameData::PlayingResponse {
                user_id, 
                beatmap_hash, 
                mode, 
                mods, 
